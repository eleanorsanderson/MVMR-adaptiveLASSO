...
educationscore <- read.delim("education.sscore", header=FALSE, comment.char="#")
cogscore <- read.delim("cognitive_ability.sscore", header=FALSE, comment.char="#")

educationscore_update <- read.delim("education_update.sscore", header=FALSE, comment.char="#")
cogscore_update <- read.delim("cognitive_ability_update.sscore", header=FALSE, comment.char="#")
...
educationscore_update <- educationscore_update %>% 
                          rename(IID = V2) %>%
                          rename(edu_grs_update = V5)

cogscore_update <- cogscore_update %>% 
                          rename(IID = V2) %>%
                          rename(cog_grs_update = V5)

scores <- educationscore %>%
  inner_join(cogscore, by="IID") %>%
  inner_join(educationscore_update, by="IID") %>%
  inner_join(cogscore_update, by="IID") %>%
  select(IID, edu_grs, cog_grs, edu_grs_update, cog_grs_update)
...
7 SNPs were removed from the education score and 3 from the cognitive ability score
